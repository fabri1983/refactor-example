...
public class CustomConsoleHandlerTest {
	...
	public void whenPublishingALogRecord_ThenItWorksOk() {
		...
		Assert.assertTrue("Published message is not the same than expected.", contains);
	}
	...
	public void whenPublishingALogRecordAfterClose_ThenItWorksOk() {
		...
		LogRecord logRecord2 = new LogRecord(Level.INFO, "A new message to StdOut after invoking close() which internally is a flush().");
		...
		Assert.assertTrue("Published messages are not the same than expected.", containsAll);
	}
